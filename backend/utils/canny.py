...
def center_crop_canny(img, **kwargs):
    height = img.shape[0]
    width = img.shape[1]
    center = [height / 2, width / 2]
    img = img[int(center[0] - height / 4): int(center[0] + height / 4),
          int(center[1] - width / 4):int(center[1] + width / 4)]
    ...
    if kwargs['single_image']==True:
        ...
        cv2.imwrite(savepath + f'/canny_{img_name}.jpg', edges)
    else:
        index = kwargs['index']
        img_name = kwargs['img_name']
        cv2.imwrite(savepath + f'/canny_{img_name}_{index}.jpg', edges)
    # cv2.waitKey(0)
